...
class MentorMenuPage(QWidget):
    ...
    def search(self):
        search_users = []
        for user in users:
            if (self.mentorMenuForm.lineEdit_mentor_username.text().lower()  in user[1].lower()\
                or self.mentorMenuForm.lineEdit_mentor_username.text().lower()  in user[2].lower())\
                and self.mentorMenuForm.lineEdit_mentor_username.text().lower()  != '':
    ...
    def update_table(self):
        ...
        if selected_row != -1:
            ...
            cell_to_update = worksheet_users.cell(selected_row + 2, col_index + 1)  # Satır ve sütun indeksleri 1'den başladığı için +2 ve +1 ekliyoruz
            cell_to_update.value = updated_value
            worksheet_users.update_cell(selected_row + 2, col_index + 1, updated_value) # Google Sheets'te hücreyi güncelle
            #self.timer.start(60000)
